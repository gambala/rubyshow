...
RSpec.describe 'Comments', type: :request do
  ...
  describe 'create comment' do
    it 'create comment for user with valid params' do
      ...
      expect( Course.find(course.id).comments.find_by(content: 'valid opinion') ).not_to eq(nil)
    end
    ...
    it 'does not create comment for user without valid params' do
      ...
      follow_redirect!
      expect(response.code).to eq('200')
      expect( Course.find(course.id).comments.find_by(content: 'invalid comment') ).to eq(nil)
    end
