...
class ApplicationRecord < ActiveRecord::Base
  ...
  def self.reject_duplicates(*, field: :id, selected_record: :last)
    query = "MIN" if selected_record == :first
    query = "MAX" if selected_record == :last
    ids = self.select("#{query}(#{field}) as #{field}").group(*).collect(&:id) # MAX(id) as id
    where(id: ids)
  end

  # User.all.select_duplicates(:email)
  # Order.all.select_duplicates(:user_id, :email, field: :slug, selected_record: :last)
  def self.select_duplicates(*, field: :id, rejected_record: :last)
